...
def menu():
    VK_USER_TOKEN = input(f'Введите свой VK токен ')
    VK_USER_ID = input(f'Введите свой VK ID с которого хотите скачать фото: ')
    folder = input(f'Укажите папку в которую хотите поместить фото: ')
    print(folder)
    photos_folder = os.path.abspath(folder)
    print(photos_folder)
    yandex_token = input(f'Введите ваш Yandex токен: ')
    user1 = VK(VK_USER_ID, VK_USER_TOKEN)
    user1.get_photo(photos_folder)
    user1 = YandexDisk(yandex_token,photos_folder)
    user1.upload()
